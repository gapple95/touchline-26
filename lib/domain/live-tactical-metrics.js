...
/**
 * Restores the squad between fixtures while retaining a small, minutes-played
 * fatigue carryover. Non-participants return at full stamina.
 */
export function applyBetweenMatchRecovery({ players, minutesByPlayerId }) {
  const safePlayers = Array.isArray(players) ? players : [];
  const safeMinutes = minutesByPlayerId && typeof minutesByPlayerId === "object" ? minutesByPlayerId : {};
  return safePlayers.map((player) => {
    const minutesPlayed = clamp(Number(safeMinutes[player.id]) || 0, 0, 120);
    const residualFatigue = Math.min(12, minutesPlayed * 0.08);
    return {
      ...player,
      stamina: Number((100 - residualFatigue).toFixed(1)),
    };
  });
}
